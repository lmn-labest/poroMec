...
def bissecao(f,x1,x2,A,tol,maxIt):    
...
def zeros(f,A,root,nRoots):
    ...
    for i in range (0,nRoots): 
        ...
        xm, stop   = bissecao(f,x1,x2,A,tol,maxIt)
...
def zeros2(f,A,root,nRoots):
    ...
    for i in range (1,nRoots): 
        x1   = i*m.pi - m.pi/2 + eps
        x2   = i*m.pi + m.pi/2 - eps
#        print (i,m.degrees(x1),m.degrees(x2))     
        xm, stop   = bissecao(f,x1,x2,A,tol,maxIt)
#       xm, stop   = newton(x2,A,tol,maxIt)
